...
class Assign(models.Model):
    ...
    def __str__(self):
        ...
        te = Teacher.objects.get(id=self.teacher.id)
        return '%s : %s : %s' % (te.user.full_name, cr.shortname, cl)
...
class AttendanceTotal(models.Model):
    ...
    @property
    def att_class(self):
        stud = Student.objects.get(user=self.student.user.id)
    ...
    def total_class(self):
        stud = Student.objects.get(user=self.student.user)
        cr = Course.objects.get(name=self.course)
